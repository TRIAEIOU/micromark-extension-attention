...
export function inlineFactory(cfg) {
  ...
  function resolveAllInline(events, context) {
    ...
    while (++index < events.length) {
      ...
      if (events[index][0] === 'enter' && events[index][1].type === tempSequence && events[index][1]._close) {
        let open = index; // Now walk back to find an opener.

        while (open--) {
          // Find a token that can open the closer.
          if (events[open][0] === 'exit' && events[open][1].type === tempSequence && events[open][1]._open && // If the sizes are the same:
          events[index][1].end.offset - events[index][1].start.offset === events[open][1].end.offset - events[open][1].start.offset) {
            ...
            const nextEvents = [['enter', inline, context], ['enter', events[open][1], context], ['exit', events[open][1], context], ['enter', text, context]]; // Between.
            ...
            splice(nextEvents, nextEvents.length, 0, [['exit', text, context], ['enter', events[index][1], context], ['exit', events[index][1], context], ['exit', inline, context]]);
  ...
  function tokenizeInline(effects, ok, nok) {
    ...
    function start(_code) {
      if (previous === _code && events[events.length - 1][1].type !== "characterEscape") {
        return nok(_code);
      }
